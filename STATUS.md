...
**Plaid Production live · multi-user infrastructure shipped + re-link unbroken.** Phase 5 dashboard fully operational. Plaid post-launch fixes (state carry-over for pending→posted re-id, 24-month history depth, initial-backfill notification silence) and infinite scroll on `/transactions` shipped. Profiles + `/settings` + per-user WhatsApp routing landed — `USER_WHATSAPP_TO` env var retired. **`/settings` now also has a QR code** for the Twilio sandbox join (scan from phone → opens WhatsApp pre-filled with `join {keyword}` → tap Send). Re-link bug fixed (exchange route used an upsert against `plaid_account_id` which couldn't address the partial unique index; switched to plain insert + made disconnect-with-wipe a hard delete that cascades). Twilio WhatsApp prod is still parked.
